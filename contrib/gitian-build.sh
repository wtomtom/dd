...
VERSION=
commit=false
url=https://github.com/vizehproject/vizeh
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/vizehproject/vizeh
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit vizeh=${COMMIT} --url vizeh=${url} ../vizeh/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../vizeh/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/vizeh-*.tar.gz build/out/src/vizeh-*.tar.gz ../vizeh-binaries/${VERSION}
	popd
