...
std::string HelpRequiringPassVZHase()
{
    return pwalletMain && pwalletMain->IsCrypted() ? "\nRequires wallet passVZHase to be set with walletpassVZHase call." : "";
}

void EnsureWalletIsUnlocked()
{
    if (pwalletMain->IsLocked() || pwalletMain->fWalletUnlockAnonymizeOnly)
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passVZHase with walletpassVZHase first.");
}
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassVZHase() +
            "\nArguments:\n"
            "1. \"vizehaddress\"  (string, required) The vizeh address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in vizeh to send. e.g. 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "      amount,                 (numeric) The amount in vizeh\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            HelpExampleCli("walletpassVZHase", "\"mypassVZHase\" 30") +
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in vizeh received at this address.\n"
            "\nExamples:\n"
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in vizeh received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 vizeh from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 vizeh timotei to akiko with a comment and funds have 6 confirmations\n" + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 6 \"happy birthday!\"") +
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            HelpRequiringPassVZHase() + "\n"
                                        ...
                                        "3. amount                (numeric, required) The amount in vizeh. (transaction fee is added on top).\n"
                                        ...
                                        "\"transactionid\"        (string) The transaction id.\n"
                                        "\nExamples:\n"
                                        "\nSend 0.01 vizeh from the default account to the address, must have at least 1 confirmation\n" +
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassVZHase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The vizeh address is the key, the numeric amount in vizeh is the value\n"
                                        "      ,...\n"
                                        "    }\n"
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in vizeh received by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in vizeh. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in vizeh. This is negative and only available for the \n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in vizeh. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in vizeh. This is negative and only available for the 'send' category of transactions.\n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in vizeh\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in vizeh\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
Value walletpassVZHase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            "walletpassVZHase \"passVZHase\" timeout ( anonymizeonly )\n"
            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
            "This is needed prior to performing transactions related to private keys such as sending VZHs\n"
            "\nArguments:\n"
            "1. \"passVZHase\"     (string, required) The wallet passVZHase\n"
            "2. timeout            (numeric, required) The time to keep the decryption key in seconds.\n"
            "3. anonymizeonly      (boolean, optional, default=flase) If is true sending functions are disabled."
            "\nNote:\n"
            "Issuing the walletpassVZHase command while the wallet is already unlocked will set a new unlock\n"
            "time that overrides the old one. A timeout of \"0\" unlocks until the wallet is closed.\n"
            "\nExamples:\n"
            "\nUnlock the wallet for 60 seconds\n" +
            HelpExampleCli("walletpassVZHase", "\"my pass VZHase\" 60") +
            "\nUnlock the wallet for 60 seconds but allow Obfuscation only\n" + HelpExampleCli("walletpassVZHase", "\"my pass VZHase\" 60 true") +
            "\nLock the wallet again (before 60 seconds)\n" + HelpExampleCli("walletlock", "") +
            "\nAs json rpc call\n" + HelpExampleRpc("walletpassVZHase", "\"my pass VZHase\", 60"));

    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassVZHase was called.");

    // Note that the walletpassVZHase is stored in params[0] which is not mlock()ed
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSVZHASE_INCORRECT, "Error: The wallet passVZHase entered was incorrect.");
...
Value walletpassVZHasechange(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            "walletpassVZHasechange \"oldpassVZHase\" \"newpassVZHase\"\n"
            "\nChanges the wallet passVZHase from 'oldpassVZHase' to 'newpassVZHase'.\n"
            "\nArguments:\n"
            "1. \"oldpassVZHase\"      (string) The current passVZHase\n"
            "2. \"newpassVZHase\"      (string) The new passVZHase\n"
            "\nExamples:\n" +
            HelpExampleCli("walletpassVZHasechange", "\"old one\" \"new one\"") + HelpExampleRpc("walletpassVZHasechange", "\"old one\", \"new one\""));

    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassVZHasechange was called.");
    ...
    if (!pwalletMain->ChangeWalletPassVZHase(strOldWalletPass, strNewWalletPass))
        throw JSONRPCError(RPC_WALLET_PASSVZHASE_INCORRECT, "Error: The wallet passVZHase entered was incorrect.");
...
Value walletlock(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            "After calling this method, you will need to call walletpassVZHase again\n"
            "before being able to call any methods which require the wallet to be unlocked.\n"
            "\nExamples:\n"
            "\nSet the passVZHase for 2 minutes to perform a transaction\n" +
            HelpExampleCli("walletpassVZHase", "\"my pass VZHase\" 120") +
            "\nPerform a send (requires passVZHase set)\n" + HelpExampleCli("sendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 1.0") +
            "\nClear the passVZHase since we are done before 2 minutes is up\n" + HelpExampleCli("walletlock", "") +
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            "encryptwallet \"passVZHase\"\n"
            "\nEncrypts the wallet with 'passVZHase'. This is for first time encryption.\n"
            "After this, any calls that interact with private keys such as sending or signing \n"
            "will require the passVZHase to be set prior the making these calls.\n"
            "Use the walletpassVZHase call for this, and then walletlock call.\n"
            "If the wallet is already encrypted, use the walletpassVZHasechange call.\n"
            "Note that this will shutdown the server.\n"
            "\nArguments:\n"
            "1. \"passVZHase\"    (string) The pass VZHase to encrypt the wallet with. It must be at least 1 character, but should be long.\n"
            "\nExamples:\n"
            "\nEncrypt you wallet\n" +
            HelpExampleCli("encryptwallet", "\"my pass VZHase\"") +
            "\nNow set the passVZHase to use the wallet, such as for signing or sending VZHs\n" + HelpExampleCli("walletpassVZHase", "\"my pass VZHase\"") +
            "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"vizehaddress\" \"test message\"") +
            "\nNow lock the wallet again by removing the passVZHase\n" + HelpExampleCli("walletlock", "") +
            "\nAs a json rpc call\n" + HelpExampleRpc("encryptwallet", "\"my pass VZHase\""));
    ...
    if (strWalletPass.length() < 1)
        throw runtime_error(
            "encryptwallet <passVZHase>\n"
            "Encrypts the wallet with <passVZHase>.");
...
Value mintzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "mintzerocoin <amount>\n"
            "Usage: Enter an amount of VZH to convert to zVZH"
            + HelpRequiringPassVZHase());

    int64_t nTime = GetTimeMillis();

    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zVZH is currently disabled due to maintenance.");
...
Value spendzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zVZH (zerocoins) into VZH. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over VZH (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zVZH
    ...
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            "exportzerocoins include_spent ( denomination )\n"
                ...
                "2. \"denomination\"         (integer, optional) Export a specific denomination of zVZH\n"
    ...
    if(fHelp || params.size() == 0)
        throw runtime_error(
            "importzerocoins importdata \n"
                ...
                "\"value\"            (string) the total zVZH value of zerocoin mints that were added\n"
...
Value reconsiderzerocoins(const Array& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
                "\nCheck archived zVZH list to see if any mints were added to the blockchain.\n"
    ...
    if(pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
                           "Error: Please enter the wallet passVZHase with walletpassVZHase first.");
