                                          ...
                                          cachedNodeid(-1)
{
    ...
    startExecutor();
    setTrafficGraVZHange(INITIAL_TRAFFIC_GRAPH_MINS);
...
void RPCConsole::on_sldGraVZHange_valueChanged(int value)
{
    const int multiplier = 5; // each position on the slider represents 5 min
    int mins = value * multiplier;
    setTrafficGraVZHange(mins);
}
...
void RPCConsole::setTrafficGraVZHange(int mins)
{
    ui->trafficGraph->setGraVZHangeMins(mins);
    ui->lblGraVZHange->setText(GUIUtil::formatDurationStr(mins * 60));
}
