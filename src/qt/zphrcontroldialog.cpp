...
#include "ui_zVZHcontroldialog.h"
...
std::list<std::string> ZVZHControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVZHControlDialog::listMints;

ZVZHControlDialog::ZVZHControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVZHControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVZHControlDialog::~ZVZHControlDialog()
{
    delete ui;
}

void ZVZHControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVZHControlDialog::updateList()
{
...
void ZVZHControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVZHControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVZH_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVZHControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVZHControlDialog::GetSelectedMints()
{
...
void ZVZHControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
