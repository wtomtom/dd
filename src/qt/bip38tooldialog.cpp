...
bool isValidPassVZHase(QString strPassVZHase, QString& strInvalid)
{
    for (int i = 0; i < strPassVZHase.size(); i++) {
        if (!validChar.contains(strPassVZHase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPassVZHase[i]))
                continue;

            strInvalid = strPassVZHase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPassVZHase = ui->passVZHaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassVZHase(qstrPassVZHase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passVZHase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qstrPassVZHase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPassVZHase = ui->passVZHaseIn_DEC->text().toStdString();
    string strKey = ui->encryptedKeyIn_DEC->text().toStdString();

    uint256 privKey;
    bool fCompressed;
    if (!BIP38_Decrypt(strPassVZHase, strKey, privKey, fCompressed)) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passVZHase and try again."));
        return;
    }
