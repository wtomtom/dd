                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVZH ought to be enough for anybody." - Bill Gates, 2017
    ui->zVZHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVZHSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVZH->setEnabled(false);
        ui->pushButtonMintzVZH->setToolTip(tr("zVZH is currently disabled due to maintenance."));

        ui->pushButtonSpendzVZH->setEnabled(false);
        ui->pushButtonSpendzVZH->setToolTip(tr("zVZH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVZHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVZH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVZH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passVZHase first."));
            return;
        }
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVZH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVZH in ") +
...
void PrivacyDialog::on_pushButtonSpendzVZH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVZH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVZH
        sendzVZH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVZH
    sendzVZH();
}

void PrivacyDialog::on_pushButtonZVZHControl_clicked()
{
    ZVZHControlDialog* zVZHControl = new ZVZHControlDialog(this);
    zVZHControl->setModel(walletModel);
    zVZHControl->exec();
}

void PrivacyDialog::setZVZHControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVZHSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVZH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid vizeh Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVZHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVZH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VZH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVZH</b>";
    ...
    // use mints from zVZH selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVZHControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVZHControlDialog::GetSelectedMints();
    }
    ...
    // Clear zVZH selector in case it was used
    ZVZHControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVZH Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " VZH, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVZH Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVZH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVZH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVZH "));
    ui->labelzVZHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVZHSyncStatus->setVisible(fShow);
}
