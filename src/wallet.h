...
// Possible states for zVZH send
enum ZerocoinSpendStatus {
    ZVZH_SPEND_OKAY = 0,                            // No error
    ZVZH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVZH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVZH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVZH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVZH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZVZH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZVZH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZVZH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZVZH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZVZH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZVZH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZVZH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZVZH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVZH_SPENT_USED_ZVZH = 14,                      // Coin has already been spend
    ZVZH_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZVZHAutoBackups(bool fEnabled)
    {
    ...
    bool Unlock(const SecureString& strWalletPassVZHase, bool anonimizeOnly = false);
    bool ChangeWalletPassVZHase(const SecureString& strOldWalletPassVZHase, const SecureString& strNewWalletPassVZHase);
    bool EncryptWallet(const SecureString& strWalletPassVZHase);
